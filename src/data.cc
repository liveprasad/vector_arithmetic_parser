...
DataHolder*  Data<T>::operator%(Data<S> * op2 ){
  ...
  if(data_type >= op2->getType()){
    ...
    Data<T>* val = new Data<T>(data_type,defination+"/"+op2->getDefination(),id,result);
	  ...
	  std::cout<<"starting divides of 2 vectors" << std::endl;
#endif
	  std::transform(vectorMeasurements->begin(),vectorMeasurements->end(),op2->getMeasurements()->begin(),back_inserter(*result),[](T x,T y){ return fmod(x,y);});
#ifdef DEB
	  std::cout<< "vector calculated for operation:"<<val->getDefination()<< std::endl;
	  std::for_each(val->getMeasurements()->begin(),val->getMeasurements()->end(),[](T val){ std::cout<<val<<" ";} );
	  std::cout<<std::endl;
    ...
    Data<S>* val = new Data<S>(data_type,defination+"/"+op2->getDefination(),id,result);
	  ...
	  std::cout<<"starting divides of 2 vectors in reverse" << std::endl;
#endif
	  std::transform(vectorMeasurements->begin(),vectorMeasurements->end(),op2->getMeasurements()->begin(),back_inserter(*result),[](S x,S y){return fmod(x,y);});
#ifdef DEB
	  std::cout<< "vector calculated for operation:"<<val->getDefination()<< std::endl;
	  std::for_each(val->getMeasurements()->begin(),val->getMeasurements()->end(),[](S val){ std::cout<<val<<" ";} );
	  std::cout<<std::endl;
...
//logical operators oveload
template<class T>
DataHolder * Data<T>::operator&(DataHolder * op2){

  if(data_type== DataType::Type::DOUBLE){
     return NULL;
  }
  switch(op2->getType()){
    case DataType::Type::INT:
...
DataHolder*  Data<T>::operator&(Data<S> * op2 ){
  ...
  if(data_type >= op2->getType()){
    ...
    Data<T>* val = new Data<T>(data_type,defination+"&"+op2->getDefination(),id,result);
#ifdef DEB
	  std::cout<<"starting logical_and of 2 vectors" << std::endl;
#endif
	  std::transform(vectorMeasurements->begin(),vectorMeasurements->end(),op2->getMeasurements()->begin(),back_inserter(*result),[](T x,T y){return int(x)&int(y);});
#ifdef DEB
	  std::cout<< "vector calculated for operation:"<<val->getDefination()<< std::endl;
	  std::for_each(val->getMeasurements()->begin(),val->getMeasurements()->end(),[](T val){ std::cout<<val<<" ";} );
	  std::cout<<std::endl;
...
Data<T>*  Data<T>::operator!( ){
#ifdef DEB
  std::cout<<"! operator overloaded for Data :" <<  "!" << defination<< "for vector size "<< vectorMeasurements->size()<<std::endl; 
#endif
  std::vector<T> *result = new std::vector<T>;
  result->reserve(vectorMeasurements->size());
  Data<T>* val = new Data<T>(data_type,"!"+defination,id,result);
...
DataHolder*  Data<T>::operator|(Data<S> * op2 ){
#ifdef DEB
  std::cout<<"| operator overloaded for Data :" << defination<< "|" << op2->getDefination()<<" with type"<< std::to_string(data_type)<< "for vector size "<< vectorMeasurements->size()<<std::endl; 
#endif
  ...
  if(data_type >= op2->getType()){
    ...
    Data<T>* val = new Data<T>(data_type,defination+"|"+op2->getDefination(),id,result);
#ifdef DEB
	  std::cout<<"starting logical_or of 2 vectors" << std::endl;
#endif
	  std::transform(vectorMeasurements->begin(),vectorMeasurements->end(),op2->getMeasurements()->begin(),back_inserter(*result),[](T x,T y){return int(x)|int(y);});
#ifdef DEB
	  std::cout<< "vector calculated for operation:"<<val->getDefination()<< std::endl;
...
DataString::DataString(std::string uuid,std::string id,std::string defination,std::vector<std::string>* vectorMeasurements){
  
  this->id = id;
  this->defination = defination;
  this->uuid= uuid;
  this->data_type = DataType::Type::STRING;
